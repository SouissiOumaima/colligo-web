...
class AuthService
{
    ...
    /**
 * Changes the password for a Parents user.
 */
/**
 * Changes the password for a Parents user.
 */
public function changePassword(Parents $user, string $currentPassword, string $newPassword): void
{
    $this->logger->info('Attempting to change password for user: {email}', ['email' => $user->getEmail()]);

    if (!$this->passwordHasher->isPasswordValid($user, $currentPassword)) {
        $this->logger->error('Change password failed: Invalid current password', ['email' => $user->getEmail()]);
    ...
    try {
        $this->entityManager->flush();
        $this->logger->info('Password changed successfully', ['email' => $user->getEmail()]);
    } catch (\Exception $e) {
        $this->logger->error('Failed to update password in database: {error}', [
            'error' => $e->getMessage(),
            'email' => $user->getEmail(),
        ]);
    ...
    try {
        ...
        $this->mailer->send($emailMessage);
        $this->logger->info('Password change confirmation email sent successfully', ['email' => $user->getEmail()]);
    } catch (\Exception $e) {
 ...
 */
public function changeAdminPassword(Admin $admin, string $currentPassword, string $newPassword): void
{
    $this->logger->info('Attempting to change password for admin: {email}', ['email' => $admin->getEmail()]);

    if (!$this->passwordHasher->isPasswordValid($admin, $currentPassword)) {
        $this->logger->error('Change password failed: Invalid current password', ['email' => $admin->getEmail()]);
        throw new \Exception('Mot de passe actuel incorrect.');
    }
    ...
    try {
        $this->entityManager->flush();
        $this->logger->info('Admin password changed successfully', ['email' => $admin->getEmail()]);
    } catch (\Exception $e) {
        $this->logger->error('Failed to update admin password in database: {error}', [
            'error' => $e->getMessage(),
    ...
    try {
        ...
        $this->mailer->send($emailMessage);
        $this->logger->info('Admin password change confirmation email sent successfully', ['email' => $admin->getEmail()]);
    } catch (\Exception $e) {
        $this->logger->error('Failed to send admin password change confirmation email: {error}', [
            'error' => $e->getMessage(),
            'email' => $admin->getEmail(),
        ]);
    }
