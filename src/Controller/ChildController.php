...
class ChildController extends AbstractController
{
    ...
    #[Route('/child/{parentId}/{childId}/progress', name: 'child_progress', methods: ['GET'])]
    public function progress(int $parentId, int $childId): Response
    {
        ...
        for ($gameId = 1; $gameId <= 5; $gameId++) {
            ...
            if ($game) {
                $levelsByGame[$gameId] = $this->levelRepository->findBy(['childId' => $child, 'gameId' => $game]);
            } else {
                $levelsByGame[$gameId] = [];
            }
        }
        ...
        for ($gameId = 1; $gameId <= 5; $gameId++) {
            $game = $this->gameRepository->find($gameId);
            $games[$gameId] = $game ? $game->getName() : "Game $gameId";
        }
